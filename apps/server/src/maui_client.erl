...
start_link(Config,Queue,ConsumerTag) when is_list(Config);
                                          is_binary(Queue);
                                          is_binary(ConsumerTag) ->
      ?INFO("Starting: ~p ~p",[Queue,ConsumerTag]),
      {ok,Pid}=gen_server:start_link({local,?SERVER},?SERVER,[Config,Queue,ConsumerTag],[]),
...
start() ->
  ...
  case maui_client:start_link(Config,RabbitQueue,RabbitConsumer) of
    {ok,Pid} -> {ok,Pid};
...
retrieve(Channel) ->
  receive
    {#'basic.deliver'{consumer_tag=_ConsumerTag
                     ...
                     ,exchange=_Exchange
                     ,routing_key=_RoutingKey},#'amqp_msg'{payload=Payload,props=Props}} ->
...
declare_publish(Exchanges,Msg,RK)
  ...
  Props=#'P_basic'{content_type=?RABBIT_CONTENT_TYPE
                  ...
                  ,headers=Headers
                  ,message_id=maui_server:generate_msg_id()
                  ,priority=?RABBIT_PRIORITY
                  ,timestamp=maui_server:time_since_epoch()
                  },
  amqp_channel:call(Channel,Publish,#amqp_msg{props=Props,payload=Msg}),
  amqp_channel:close(Channel),
  amqp_connection:close(Connection),
  ok.
