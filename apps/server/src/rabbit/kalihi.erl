...
-define(EVENTS_TIMEOUTS, 9_999).

-include("./_build/default/lib/amqp_client/include/amqp_client.hrl").

-record(state, {connection, channel}).

timeout_millseconds() -> ?EVENTS_TIMEOUTS.
...
handle_info(timeout, State = #state{channel = Channel}) ->
  ...
  amqp_channel:cast(Channel, #'basic.publish'{exchange = ?EVENTS_EXCHANGE, routing_key = ?EVENTS_EXCHANGE}, #amqp_msg{payload = ?EVENTS_PAYLOADS}),
